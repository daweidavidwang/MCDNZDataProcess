function [inputs, labels, set] = mat_patches_generation3(size_input,size_label,stride,folder,mode,max_numPatches,batchSize)
...
%     input_exr(input_exr>1) = 1;
    ...
    meanTmp = mean(input_exr(:));
    clear input_exr;

    image = cat(3,image,input_im(:,:,1:3),medfilt2(input_im(:,:,1)),medfilt2(input_im(:,:,2)),medfilt2(input_im(:,:,3)) );%将input图像放入GT中，用于残差的相加操作
    
    for j = 4:16
        tmp = input_im(:,:,j);
        input_im(:,:,j) = meanTmp*input_im(:,:,j)/mean(tmp(:));
...
%                 for k =1:16
%                     tmp = subim_input(:,:,k);
%                     subim_input(:,:,k) = ( subim_input(:,:,k)-mean(tmp(:)) )/std(tmp(:));
%                 end
...
inputsMean = zeros(size(inputs,4),1);
for index = 1:size(inputs,4)
    inputsMean(index) = mean(mean(mean( inputs(:,:,1:3,index)  )));
end
[~,rank] = sort(inputsMean);
rankStride = size(inputs,4)/(batchSize/2);
a = sort(repmat((1:rankStride),1,(batchSize/2)));
zz = (0:(batchSize/2)-1);
zz = zz(randperm(batchSize/2));
b = rankStride*repmat(zz,1,rankStride);
