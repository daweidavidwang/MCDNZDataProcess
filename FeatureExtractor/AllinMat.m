...
folderMat = 'G:\DatasetWithSecondFeature_aug3\4SPP\Feature';
%folderJpg = 'G:\DatasetWithSecondFeature_aug3\64SPP\varexr\jpg';
folderNewFeature = 'G:\DatasetWithSecondFeature_aug3\4SPP\AllinMat';
foldernoise = 'G:\DatasetWithSecondFeature_aug3\4SPP\exr';
filepathGt = [];
ext               =  {'*.exr'};
for i = 1 : length(ext)
...
for i = 1 : length(filepathGt)
     ...
     inputimage = exrread(fullfile(foldernoise,[filepathGt(i).name(1:end-4),'_MC_0004','.exr']));
     ...
     data(:,:,39) = gxtex2;
     data(:,:,40) = gytex2;
     save(fullfile(folderNewFeature,[filepathGt(i).name(1:end-4),'.mat']),'data');
end