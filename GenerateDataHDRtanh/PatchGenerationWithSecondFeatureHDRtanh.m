function [inputs, labels, set] = PatchGenerationWithSecondFeatureHDRtanh(alpha,FeatureMode,size_input,size_label,stride,folder,folderpa,mode,max_numPatches,batchSize)
PatchSelect = 0;
%featureNum = 17;
%alpha = 0.4361; %%HDR normalization tanh(alpha*input)
...
if FeatureMode == 1
    inputs  = zeros(size_input, size_input, 17, 1,'single');
...
if FeatureMode == 2
    inputs  = zeros(size_input, size_input, 13, 1,'single');
    labels  = zeros(size_label, size_label, 6, 1,'single');
    count   = 0;
    padding = abs(size_input - size_label)/2;
    for i = 1 : length(filepathsGt)
    ...
    for index = 1:length(spp) 
        image = exrread(fullfile(folderGt,filepathsGt(i).name))  ;
        folderData = fullfile(folder,spp(index));
        folderDataFeature = fullfile(folderData,'SobelCVarN');
        folderData = fullfile(folderData,'exr');
        tmp = fullfile(folderData,[filepathsGt(i).name(1:end-4),jpgName(index,:)] );
        input_jpg =  exrread( char(tmp) );
        featureinput = load( char(fullfile(folderDataFeature,[filepathsGt(i).name(1:end-4),'.mat'] )) );
        input_jpg = tanh(alpha*input_jpg);
        input_im = cat(3,input_jpg,featureinput.SobelCVarN(:,:,1:10));
