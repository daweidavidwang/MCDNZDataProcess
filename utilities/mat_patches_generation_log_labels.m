function [labels] = mat_patches_generation_log_labels(size_input,size_label,stride,folder,mode,max_numPatches,batchSize,subNum,order)

featureNum = 16;
inputs  = zeros(size_input, size_input, featureNum, 1,'single');
labels  = zeros(size_label, size_label, 9, 248000,'single');
count   = 0;
...
for i = 1 : length(filepathsGt)
    ...
    for index = 1:length(spp) 
        ...
        for j = 1:1
            ...
            clear im_label;clear im_data; clear subim_input; clear subim_label;clear inputs;
        end
    end
